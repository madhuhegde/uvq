"""Pytorch implementation of the DistortionNet for UVQ 1.5 without depthwise convolution.

This is a modified version that uses normal Conv2D instead of depthwise Conv2D
to avoid TFLite conversion issues.
...
from . import custom_nn_layers_no_depthwise as custom_nn_layers
...
class DistortionNetCore(nn.Module):
  """DistortionNet is based on the EfficientNet architecture (without depthwise conv).

  This version uses normal Conv2D instead of depthwise Conv2D to avoid TFLite issues.
  """
...
class DistortionNet(nn.Module):
  """Pytorch implementation of the DistortionNet for UVQ 1.5 (without depthwise conv).
